...
class EasyTimetableGA:
    def __init__(self, courses: List[Course], instructors: List[Instructor], rooms: List[Room]):
        ...
        # หากต้องการให้ Lec/Lab อยู่วันเดียวกัน เปลี่ยนค่านี้เป็น 1
    ...
    def _biased_time_choices(self, course: Course, day: str):
        ...
        if inst:
            ...
            if pref == "morning":
                base = self.times[:4]*3 + self.times[4:]
            elif pref == "afternoon":
                base = self.times[4:]*3 + self.times[:4]
    ...
    def _try_place_block(self, occ, course, day, start_idx, room):
        """วางบล็อก consecutive โดยกันห้องปิดใช้งานด้วย"""
        need = course.requires_consecutive
        if start_idx + need > len(self.times): return None
        if (self.lunch_break_index is not None and
            start_idx <= self.lunch_break_index < start_idx + need):
            return None
        slots = []
        for k in range(need):
            t = self.times[start_idx + k]
            ts = f"{day} {t}"
            if (ts in occ['instr'].get(course.instructor_name, set()) or
                ts in occ['room'].get(room.name, set()) or
                ts in getattr(room, 'unavailable_times', []) or
    ...
    def create_heuristic_schedule(self) -> List[Dict]:
        ...
        sorted_courses = sorted(
            self.courses,
            key=lambda c: (len(self.compatible_rooms[c.id]), -c.requires_consecutive, -c.students)
        )
        for course in sorted_courses:
            placed_periods = 0
            # วางบล็อก consecutive ก่อน (กันห้องปิดใช้งานไว้ใน _try_place_block แล้ว)
            if course.requires_consecutive > 1:
                for _ in range(20):
                    day = random.choice(self.days)
                    room = random.choice(self.compatible_rooms[course.id])
                    ...
                    if block:
                        ...
                        placed_periods = course.requires_consecutive
                        break
            # วางคาบที่เหลือแบบ bias เวลา -> เลือกห้องที่ไม่ปิดช่วงนั้น
            remaining_periods = range(placed_periods + 1, course.periods_per_week + 1)
            for p_num in remaining_periods:
                class_id = f"{course.id}_period{p_num}"
                placed = False
                for _ in range(self.HEURISTIC_PLACEMENT_ATTEMPTS):
                    day = random.choice(self.days)
                    time = random.choice(self._biased_time_choices(course, day))
                    cand_rooms = [r for r in self.compatible_rooms[course.id]
                                  if f"{day} {time}" not in r.unavailable_times]
                    room = random.choice(cand_rooms) if cand_rooms else random.choice(self.compatible_rooms[course.id])
                    tslot = f"{day} {time}"
                    if (tslot not in occupancy['instr'].get(course.instructor_name, set()) and
                        ...
                        break
                if not placed:
                    # fallback: วางสุ่ม (ยอมรับว่าอาจ conflict) เพื่อให้ครบยีน
                    room = random.choice(self.compatible_rooms[course.id])
                    day = random.choice(self.days)
    ...
    def check_fitness(self, schedule: List[Dict]) -> float:
        ...
        for cl_info in schedule:
            ...
            tslot = f"{cl_info['day']} {cl_info['time']}"
            inst, room_name = course.instructor_name, cl_info['room']

            # Instructor conflicts
            if tslot in occupancy['instr'].setdefault(inst, set()):
                score -= self.weights['instr_conflict']
            else:
                occupancy['instr'][inst].add(tslot)

            # Room conflicts
            if tslot in occupancy['room'].setdefault(room_name, set()):
                score -= self.weights['room_conflict']
            else:
                occupancy['room'][room_name].add(tslot)

            # Cohort conflicts
            for cohort_id in course.cohort_ids:
                if tslot in occupancy['cohort'].setdefault(cohort_id, set()):
                    score -= self.weights['cohort_conflict']
                else:
                    occupancy['cohort'][cohort_id].add(tslot)

            # Instructor unavailable & preferences
            if inst in self.instructors:
                if tslot in self.instructors[inst].unavailable_times:
                    score -= self.weights['unavailable']
                pref = (self.instructors[inst].preferred or "").lower()
                if pref == "morning":
                    score += self.weights['pref_bonus'] if cl_info['time'] in self.morning_times else -self.weights['pref_penalty']
                elif pref == "afternoon":
                    score += self.weights['pref_bonus'] if cl_info['time'] in self.afternoon_times else -self.weights['pref_penalty']

            # Room attributes
            room = self.rooms_map.get(room_name)
            if room:
                if tslot in room.unavailable_times:
                    score -= self.weights['room_unavailable']
                if room.room_type != course.course_type:
                    score -= self.weights['wrong_room_type']
                if room.capacity < course.students:
                    score -= self.weights['over_capacity'] * (1 + (course.students - room.capacity) / max(1, room.capacity))

            per_course_day.setdefault((cl_info['course_id'], cl_info['day']), []).append(self.time_index[cl_info['time']])
        
        # Consecutive & same-course-same-day penalties/bonus
        for (course_id, day), idx_list in per_course_day.items():
            course = self.course_map[course_id]
            idx_list.sort()
            if course.requires_consecutive > 1:
                score += self.weights['consecutive_bonus'] if self.has_run_at_least(idx_list, course.requires_consecutive) \
                        else -self.weights['consecutive_penalty']
        ...
        # Linked courses: กระจายวัน + Lab หลัง Lec (ใช้ course_type แทนการดู 'Lab' ในชื่อ)
        linked_courses = {}
        for course_id in self.course_map:
            linked_courses.setdefault(self._base_id(course_id), []).append(course_id)
        
        days_by_cid = {}
        for (cid, day), idx_list in per_course_day.items():
            if idx_list:
                days_by_cid.setdefault(cid, set()).add(day)
        ...
        for base_id, group in linked_courses.items():
            ...
            if len(days_used) < required:
                score -= self.weights['linked_course_same_day']
            
            lec_course_id = next((c for c in group if self.course_map[c].lab_must_be_after_lec), None)
            lab_course_id = next((c for c in group if self.course_map[c].course_type == 'lab'), None)
            if lec_course_id and lab_course_id:
    ...
    def _check_gene_conflict(self, gene: Dict, occupancy: Dict) -> bool:
        course = self.course_map[gene['course_id']]
        ts = f"{gene['day']} {gene['time']}"
    ...
    def greedy_crossover(self, p1: List[Dict], p2: List[Dict]) -> List[Dict]:
        """รวมยีนจากทั้งสองพ่อแม่ + เลี่ยง conflict ระหว่างประกอบ"""
        m1 = {g['class_id']: g for g in p1}
        m2 = {g['class_id']: g for g in p2}
        all_ids = list(set(m1.keys()) | set(m2.keys()))  # รวมยีนทุกตัว
        child, occ = [], {'instr': {}, 'room': {}, 'cohort': {}}
        for cid in all_ids:
            g1 = m1.get(cid, m2[cid])
            g2 = m2.get(cid, g1)
            ...
            if not conflict1 and (conflict2 or random.random() < 0.5):
                ...
                # ทั้งคู่ชน: ลองขยับเวลาใกล้เคียงเพื่อลด conflict
                selected = dict(g1)
                neighbors = self._neighbor_times(selected['time'], exclude_self=True)
                if neighbors:
                    selected['time'] = random.choice(neighbors)
    ...
    def mutate(self, sched, rate=0.1):
        for i in range(len(sched)):
            if random.random() < rate:
                gene = random.choice(['day', 'time', 'room'])
                course = self.course_map[sched[i]['course_id']]
                if gene == 'room':
                    sched[i]['room'] = random.choice(self.compatible_rooms[course.id]).name
                elif gene == 'day':
                    sched[i]['day'] = random.choice(self.days)
                elif gene == 'time':
                    sched[i]['time'] = random.choice(self._biased_time_choices(course, sched[i]['day']))
        return sched
    
    def _repair(self, sched):
        for x in sched:
            course = self.course_map[x['course_id']]
            room = self.rooms_map.get(x['room'])
    ...
    def _is_part_of_good_run(self, schedule, class_info, run_cache):
        course_id, day = class_info['course_id'], class_info['day']
        key = (course_id, day)
        ...
        if course.requires_consecutive <= 1:
            run_cache[key] = True
            return True
        indices = sorted([self.time_index[x['time']] for x in schedule
                          if x['course_id'] == course_id and x['day'] == day])
        result = self.has_run_at_least(indices, course.requires_consecutive)
    ...
    def _snap_to_run_time(self, schedule, class_info):
        ...
        other_indices = sorted([self.time_index[x['time']] for x in schedule
                                if x['course_id'] == course_id and x['day'] == day and x is not class_info])
        if not other_indices: return None
        for start in range(len(self.times) - course.requires_consecutive + 1):
            if self.lunch_break_index is not None and start <= self.lunch_break_index < start + course.requires_consecutive:
                continue
            ...
            if len(missing_indices) == 1:
                return self.times[missing_indices[0]]
        return None
    ...
    def _repair_conflicts(self, sched):
        ...
        for x in sched:
            ...
            is_conflicted = (keys[0] in seen_keys[0] or
                             keys[1] in seen_keys[1] or
                             any((cid, ts) in seen_keys[2] for cid in course.cohort_ids))
        ...
        for x in bad_genes:
            ...
            if snapped_time:
                ts = f"{x['day']} {snapped_time}"
                keys = ((course.instructor_name, ts), (x['room'], ts))
                if not (keys[0] in seen_keys[0] or keys[1] in seen_keys[1] or
                        any((cid, ts) in seen_keys[2] for cid in course.cohort_ids)):
                    x['time'] = snapped_time
                    fixed = True
                    seen_keys[0].add(keys[0]); seen_keys[1].add(keys[1])
            ...
            # 4) ✅ ลองเปลี่ยนห้อง (ที่ compatible และไม่ปิดช่วงนั้น)
            if not fixed:
                for _ in range(4):
                    new_room_obj = random.choice(self.compatible_rooms[course.id])
                    ts = f"{x['day']} {x['time']}"
                    if (ts not in new_room_obj.unavailable_times and
                        (course.instructor_name, ts) not in seen_keys[0] and
                        (new_room_obj.name, ts) not in seen_keys[1] and
                        not any((cid, ts) in seen_keys[2] for cid in course.cohort_ids)):
                        x['room'] = new_room_obj.name
                        seen_keys[0].add((course.instructor_name, ts))
                        seen_keys[1].add((new_room_obj.name, ts))
                        for cid in course.cohort_ids: seen_keys[2].add((cid, ts))
                        fixed = True
                        break
    ...
    def evolve(self, population_size=80, generations=1000, mutate_rate=0.2, patience=120, elitism=5, seed: Optional[int]=None, cancel_flag=None):
        ...
        best_ever_score, stale_generations = -float('inf'), 0
        best_schedule = population[0]
        immigrants = max(1, population_size // 20)  # 5%
        ...
        for gen in range(generations):
            ...
            if gen % 50 == 0:
                print(f"Generation {gen}: Best Score = {best_ever_score:.1f} (Stagnant for {stale_generations} gens)")

            if stale_generations >= patience and best_ever_score > 0:
                print(f"✅ Stopping at generation {gen} due to stagnation (Best: {best_ever_score:.1f})")
                break
            ...
            while len(pop_new) < population_size - immigrants:
                ...
                child = self.greedy_crossover(p1, p2)
                child = self.mutate(child, mutate_rate)
            ...
            for _ in range(immigrants):
                pop_new.append(self.create_heuristic_schedule())
    ...
    def show_grid(self, root, schedule: List[Dict]):
        ...
        for c, day in enumerate(self.days, start=1):
            tk.Label(table, text=day, borderwidth=1, relief="solid", font=("", 10, "bold")).grid(row=0, column=c, sticky="nsew")
        ...
        for r, t in enumerate(self.times, start=1):
            ...
            for c, d in enumerate(self.days, start=1):
                ...
                if items:
                    ...
                    insts  = [it['course'].instructor_name for it in items]
                    rooms  = [it['room'] for it in items]
                    all_cohorts = [item for sublist in cohorts for item in sublist]
                    has_hard_conflict = is_multi_slot and (
                        len(set(all_cohorts)) < len(all_cohorts) or
                        len(set(insts)) < len(items) or
                        len(set(rooms)) < len(items)
                    )
                    bg = "#ffcccc" if has_hard_conflict else ("#ffebcc" if is_multi_slot else "#e8f4ff")
                    txt = "\n".join(f"{it['course'].name}\n({', '.join(it['course'].cohort_ids)})\n{it['course'].instructor_name} • {it['room']}"
                                    for it in items)
                lbl = tk.Label(table, text=txt, borderwidth=1, relief="solid",
                               justify="left", anchor="nw", padx=6, pady=6, bg=bg, wraplength=wrap)
...
def create_easy_data():
    instructors = [
        Instructor("Inst. Sombat", [], ""),
        Instructor("Inst. Jintana", [], "morning"),
        Instructor("Inst. Wichan", ["Friday 08:00", "Friday 09:00"], "morning"),
        Instructor("Inst. Somying", [], "afternoon"),
        Instructor("Inst. Prapai", [], "afternoon"),
    ]
    rooms = [
        Room("LEC101", 100, "lecture", unavailable_times=["Wednesday 13:00", "Wednesday 14:00"]),
        Room("LEC202", 50, "lecture"),
        Room("LAB301", 30, "lab"),
        Room("LAB302", 30, "lab"),
        Room("LAB303", 50, "lab"),
    ]
...
class DataEntryUI(tk.Toplevel):
    def __init__(self, master, days, times):
        ...
        self.title("Timetable GA (University Pro)")
        self.geometry("1250x720")
        self.days, self.times = days, times
        self.instructors, self.rooms, self.courses = [], [], []
        ...
        tk.Label(topbar, text="Pop:").pack(side=tk.LEFT, padx=(10,0))
        self.pop_var = tk.IntVar(value=100)
        tk.Spinbox(topbar, from_=20, to=500, textvariable=self.pop_var, width=5).pack(side=tk.LEFT)

        tk.Label(topbar, text="Gen:").pack(side=tk.LEFT, padx=(10,0))
        self.gen_var = tk.IntVar(value=1000)
        tk.Spinbox(topbar, from_=100, to=5000, textvariable=self.gen_var, width=6).pack(side=tk.LEFT)

        tk.Label(topbar, text="Mutate:").pack(side=tk.LEFT, padx=(10,0))
        self.mut_var = tk.DoubleVar(value=0.2)
        tk.Spinbox(topbar, from_=0.01, to=1.0, increment=0.01, textvariable=self.mut_var, width=5).pack(side=tk.LEFT)

        tk.Label(topbar, text="Patience:").pack(side=tk.LEFT, padx=(10,0))
        self.patience_var = tk.IntVar(value=150)
        tk.Spinbox(topbar, from_=20, to=500, textvariable=self.patience_var, width=5).pack(side=tk.LEFT)

        tk.Label(topbar, text="Seed:").pack(side=tk.LEFT, padx=(10,0))
        self.seed_var = tk.StringVar(value="")
        tk.Entry(topbar, textvariable=self.seed_var, width=6).pack(side=tk.LEFT)

        self.generate_btn = tk.Button(topbar, text="🚀 Generate", command=self.on_generate, bg="#1e90ff", fg="white", font=("", 10, "bold"))
        self.generate_btn.pack(side=tk.LEFT, padx=10)

        self._cancel_flag = threading.Event()
        self.cancel_btn = tk.Button(topbar, text="Cancel", command=lambda: self._cancel_flag.set(), bg="#ffaaaa", state="disabled")
        ...
        for name, builder in tabs.items():
            tab = tk.Frame(nb); nb.add(tab, text=name); builder(tab)

        self.update_course_instructor_options()
        self.queue = queue.Queue()

    def build_instructors_tab(self, parent):
        left = tk.Frame(parent); left.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)
        right = tk.Frame(parent); right.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)

        cols = ("name", "preferred", "unav_count")
        self.inst_tree = ttk.Treeview(left, columns=cols, show="headings")
        self.inst_tree.heading("name", text="Instructor Name")
        self.inst_tree.heading("preferred", text="Preference")
        self.inst_tree.heading("unav_count", text="Unavailable (#)")
        self.inst_tree.pack(fill=tk.BOTH, expand=True, pady=5)
        self.inst_tree.bind("<<TreeviewSelect>>", self.on_inst_select)

        btns = tk.Frame(left); btns.pack(fill=tk.X, pady=5)
        tk.Button(btns, text="Add/Update", command=self.add_update_instructor).pack(side=tk.LEFT)
        tk.Button(btns, text="Delete", command=self.delete_instructor).pack(side=tk.LEFT, padx=5)
        tk.Button(btns, text="Clear Form", command=self.clear_inst_form).pack(side=tk.LEFT)

        tk.Label(right, text="Instructor Name").pack(anchor="w")
        self.inst_name_var = tk.StringVar()
        tk.Entry(right, textvariable=self.inst_name_var, width=30).pack(pady=(0,5))

        tk.Label(right, text="Time Preference").pack(anchor="w")
        self.inst_pref_var = tk.StringVar(value="")
        ttk.Combobox(right, textvariable=self.inst_pref_var, values=["", "morning", "afternoon"], state="readonly").pack(pady=(0,5))

        tk.Label(right, text="Unavailable Times").pack(anchor="w", pady=(10,0))
        grid = tk.Frame(right, borderwidth=1, relief="groove"); grid.pack()
        self.inst_unav_vars = {}
        ...
        for r, t in enumerate(self.times):
            for c, d in enumerate(self.days):
                var = tk.BooleanVar()
                tk.Checkbutton(grid, variable=var).grid(row=r+1, column=c+1)
                self.inst_unav_vars[(d,t)] = var

    def build_rooms_tab(self, parent):
        left = tk.Frame(parent); left.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)
        right = tk.Frame(parent); right.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)

        cols = ("name", "capacity", "type", "unav_count")
        self.room_tree = ttk.Treeview(left, columns=cols, show="headings")
        ...
        self.room_tree.heading("unav_count", text="Unavailable (#)")
        self.room_tree.pack(fill=tk.BOTH, expand=True, pady=5)
        self.room_tree.bind("<<TreeviewSelect>>", self.on_room_select)

        btns = tk.Frame(left); btns.pack(fill=tk.X, pady=5)
        tk.Button(btns, text="Add/Update", command=self.add_update_room).pack(side=tk.LEFT)
        tk.Button(btns, text="Delete", command=self.delete_room).pack(side=tk.LEFT, padx=5)
        tk.Button(btns, text="Clear Form", command=self.clear_room_form).pack(side=tk.LEFT)

        tk.Label(right, text="Room Name").pack(anchor="w")
        self.room_name_var = tk.StringVar()
        tk.Entry(right, textvariable=self.room_name_var, width=30).pack(pady=(0,5))

        tk.Label(right, text="Capacity").pack(anchor="w")
        self.room_cap_var = tk.IntVar(value=30)
        tk.Spinbox(right, from_=1, to=500, textvariable=self.room_cap_var).pack(pady=(0,5))

        tk.Label(right, text="Room Type").pack(anchor="w")
        self.room_type_var = tk.StringVar(value="lecture")
        ttk.Combobox(right, textvariable=self.room_type_var, values=["lecture", "lab"], state="readonly").pack(pady=(0,5))
        ...
        for r, t in enumerate(self.times):
            for c, d in enumerate(self.days):
                var = tk.BooleanVar()
                tk.Checkbutton(grid, variable=var).grid(row=r+1, column=c+1)
                self.room_unav_vars[(d,t)] = var

    def build_courses_tab(self, parent):
        left = tk.Frame(parent); left.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)
        right = tk.Frame(parent); right.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)

        cols = ("id", "name", "inst", "cohort", "type", "students", "periods", "consecutive")
        self.course_tree = ttk.Treeview(left, columns=cols, show="headings")
        for col, width, anchor in [
            ("id",80,'w'), ("name",150,'w'), ("inst",120,'w'), ("cohort",100,'w'),
            ("type",60,'w'), ("students",60,'center'), ("periods",60,'center'), ("consecutive",60,'center')
        ]:
            self.course_tree.heading(col, text=col.capitalize())
            self.course_tree.column(col, width=width, anchor=anchor)
        self.course_tree.pack(fill=tk.BOTH, expand=True, pady=5)
        self.course_tree.bind("<<TreeviewSelect>>", self.on_course_select)

        btns = tk.Frame(left); btns.pack(fill=tk.X, pady=5)
        tk.Button(btns, text="Add/Update", command=self.add_update_course).pack(side=tk.LEFT)
        tk.Button(btns, text="Delete", command=self.delete_course).pack(side=tk.LEFT, padx=5)
        tk.Button(btns, text="Clear Form", command=self.clear_course_form).pack(side=tk.LEFT)

        form = tk.Frame(right); form.pack()
        tk.Label(form, text="Course ID (e.g., CS101_Lec)").grid(row=0, column=0, sticky="w")
        self.course_id_var = tk.StringVar()
        tk.Entry(form, textvariable=self.course_id_var).grid(row=1, column=0, columnspan=2, sticky="ew", pady=(0,5))

        tk.Label(form, text="Course Name").grid(row=2, column=0, sticky="w")
        self.course_name_var = tk.StringVar()
        tk.Entry(form, textvariable=self.course_name_var, width=30).grid(row=3, column=0, columnspan=2, sticky="ew", pady=(0,5))

        tk.Label(form, text="Instructor").grid(row=4, column=0, sticky="w")
        self.course_inst_var = tk.StringVar()
        self.inst_combo = ttk.Combobox(form, textvariable=self.course_inst_var, state="readonly")
        self.inst_combo.grid(row=5, column=0, columnspan=2, sticky="ew", pady=(0,5))

        tk.Label(form, text="Cohort IDs (comma-separated)").grid(row=6, column=0, sticky="w")
        self.course_cohort_var = tk.StringVar()
        tk.Entry(form, textvariable=self.course_cohort_var).grid(row=7, column=0, columnspan=2, sticky='ew', pady=(0,5))

        tk.Label(form, text="Students").grid(row=8, column=0, sticky="w")
        self.course_stud_var = tk.IntVar(value=30)
        tk.Spinbox(form, from_=1, to=500, textvariable=self.course_stud_var, width=10).grid(row=9, column=0, sticky='w', pady=(0,5))

        tk.Label(form, text="Course Type").grid(row=8, column=1, sticky="w")
        self.course_type_var = tk.StringVar(value="lecture")
        ttk.Combobox(form, textvariable=self.course_type_var, values=["lecture", "lab"], state="readonly", width=10).grid(row=9, column=1, sticky='w', pady=(0,5))

        tk.Label(form, text="Periods/Week").grid(row=10, column=0, sticky="w")
        self.course_periods_var = tk.IntVar(value=2)
        tk.Spinbox(form, from_=1, to=10, textvariable=self.course_periods_var, width=10).grid(row=11, column=0, sticky='w')

        tk.Label(form, text="Consecutive").grid(row=10, column=1, sticky="w")
        self.course_consecutive_var = tk.IntVar(value=2)
        tk.Spinbox(form, from_=1, to=5, textvariable=self.course_consecutive_var, width=10).grid(row=11, column=1, sticky='w')

        self.course_dependency_var = tk.BooleanVar()
        tk.Checkbutton(form, text="Lab must be after Lec", variable=self.course_dependency_var).grid(row=12, column=0, columnspan=2, sticky='w', pady=5)
    ...
    def on_inst_select(self, _=None):
        ...
        self.inst_name_var.set(data['name'])
        self.inst_pref_var.set(data['preferred'])
        for (d,t), var in self.inst_unav_vars.items():
            var.set(f"{d} {t}" in data['unavailable_times'])
    
    def add_update_instructor(self):
        name = self.inst_name_var.get().strip()
        if not name:
            return messagebox.showwarning("Input Error", "Instructor name cannot be empty.")
        ...
        if sel:
            self.instructors[self.inst_tree.index(sel[0])] = data
        else:
            if any(i['name'] == name for i in self.instructors):
                return messagebox.showwarning("Duplicate", f"Instructor '{name}' already exists.")
            self.instructors.append(data)
        self.refresh_inst_tree()
        self.clear_inst_form()
        self.update_course_instructor_options()
    ...
    def refresh_inst_tree(self):
        ...
        for item in self.instructors:
            self.inst_tree.insert('', 'end', values=(item['name'], item['preferred'], len(item['unavailable_times'])))
    ...
    def on_room_select(self, _=None):
        ...
        self.room_name_var.set(data['name'])
        self.room_cap_var.set(data['capacity'])
        self.room_type_var.set(data['room_type'])
        for (d,t), var in self.room_unav_vars.items():
            var.set(f"{d} {t}" in data.get('unavailable_times', []))

    def add_update_room(self):
        name = self.room_name_var.get().strip()
        if not name:
            return messagebox.showwarning("Input Error", "Room name cannot be empty.")
        ...
        if sel:
            self.rooms[self.room_tree.index(sel[0])] = data
        else:
            if any(r['name'] == name for r in self.rooms):
                return messagebox.showwarning("Duplicate", f"Room '{name}' already exists.")
            self.rooms.append(data)
        self.refresh_room_tree()
        self.clear_room_form()

    def delete_room(self):
        if not self.room_tree.selection(): return
        self.rooms.pop(self.room_tree.index(self.room_tree.selection()[0]))
        self.refresh_room_tree()
        self.clear_room_form()
    
    def clear_room_form(self):
        self.room_name_var.set('')
        self.room_cap_var.set(30)
        self.room_type_var.set('lecture')
        self.room_tree.selection_remove(self.room_tree.selection())
    ...
    def refresh_room_tree(self):
        ...
        for item in self.rooms:
            self.room_tree.insert('', 'end', values=(item['name'], item['capacity'], item['room_type'], len(item.get('unavailable_times', []))))
    ...
    def on_course_select(self, _=None):
        ...
        self.course_id_var.set(data['id'])
        self.course_name_var.set(data['name'])
        self.course_inst_var.set(data['instructor_name'])
        self.course_cohort_var.set(", ".join(data['cohort_ids']))
        self.course_stud_var.set(data['students'])
        self.course_type_var.set(data['course_type'])
        self.course_periods_var.set(data['periods_per_week'])
        self.course_consecutive_var.set(data['requires_consecutive'])
    ...
    def add_update_course(self):
        ...
        inst_name = self.course_inst_var.get()
        if not cid or not inst_name:
            return messagebox.showwarning("Input Error", "Course ID and Instructor are required.")
        if '_' not in cid:
            return messagebox.showwarning("Input Error", f"Course ID '{cid}' must have a suffix, e.g., _Lec or _Lab")
        if inst_name not in [i['name'] for i in self.instructors]:
            return messagebox.showwarning("Input Error", f"Instructor '{inst_name}' does not exist.")
        cohorts_str = self.course_cohort_var.get().strip()
        if not cohorts_str:
            return messagebox.showwarning("Input Error", "Cohort ID(s) cannot be empty.")
        ...
        if sel:
            self.courses[self.course_tree.index(sel[0])] = data
        else:
            if any(c['id'] == cid for c in self.courses):
                return messagebox.showwarning("Duplicate", f"Course ID '{cid}' already exists.")
    ...
    def delete_course(self):
        if not self.course_tree.selection(): return
        self.courses.pop(self.course_tree.index(self.course_tree.selection()[0]))
        self.refresh_course_tree()
        self.clear_course_form()
    ...
    def refresh_course_tree(self):
        ...
        for c in self.courses: 
            self.course_tree.insert('', 'end', values=(
                c['id'], c['name'], c['instructor_name'], ", ".join(c['cohort_ids']), 
    ...
    def on_generate(self):
        if not self.courses:
            return messagebox.showwarning("No Data", "Please add courses before generating a schedule.")
        self._cancel_flag.clear()
        self.generate_btn.config(state="disabled", text="Working...")
        self.cancel_btn.config(state="normal")
        self.busy_win = tk.Toplevel(self)
        self.busy_win.title("Processing...")
        tk.Label(self.busy_win, text="GA is evolving... Please wait.\nThis window will close automatically.").pack(padx=30, pady=20)
    ...
    def _check_ga_result(self):
        try:
            ...
            if hasattr(self, 'busy_win') and self.busy_win.winfo_exists():
